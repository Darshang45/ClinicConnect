...
function PharmacistDetails({ pharmacist, onBack, onEdit, onDelete }) {
  if (!pharmacist) {
    return (
      <Card>
        <CardBody className="text-center py-5">
          <h5>No pharmacist selected.</h5>
  ...
  const initials = (pharmacist.fullName || "P")
    .split(" ")
    .map((n) => n[0])
    .join("")
    .substring(0, 2)
  ...
  return (
    <div className="staff-details-page">
      <PageHeader
        ...
        subtitle={`Viewing profile record for ${pharmacist.fullName}`}
      >
        ...
        <div className="col-lg-4 col-12">
          <Card className="text-center h-100">
            <CardBody className="d-flex flex-column align-items-center justify-content-center py-5">
              ...
              <h4 className="fw-bold mb-1">{pharmacist.fullName}</h4>
              <p className="text-muted small mb-2">Pharmacy Manager</p>
              <Badge variant={pharmacist.isActive !== false ? "success" : "danger"} showDot className="mb-3 fs-6">
                {pharmacist.isActive !== false ? "Active Staff Account" : "Inactive"}
              </Badge>
            </CardBody>
          </Card>
        </div>
              ...
              <div className="row g-3">
                <div className="col-sm-6 col-12">
                  <div className="p-3 bg-light rounded-3">
                    ...
                    <strong className="text-dark fs-6">{pharmacist.fullName}</strong>
                  </div>
                </div>
                <div className="col-sm-6 col-12">
                  <div className="p-3 bg-light rounded-3">
                    <span className="text-muted small d-block">Email Address</span>
                    <strong className="text-dark fs-6">{pharmacist.email}</strong>
                  </div>
                </div>
                <div className="col-sm-6 col-12">
                  <div className="p-3 bg-light rounded-3">
                    <span className="text-muted small d-block">Phone Number</span>
                    <strong className="text-dark fs-6">{pharmacist.phone}</strong>
                  </div>
                </div>
                <div className="col-sm-6 col-12">
                  <div className="p-3 bg-light rounded-3">
                    <span className="text-muted small d-block">System Role</span>
                    <strong className="text-dark fs-6">Pharmacist</strong>
                  </div>
                </div>
              </div>
            </CardBody>
