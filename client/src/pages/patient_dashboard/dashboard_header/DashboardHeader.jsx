...
const accountNavigation = [{ label: "Profile", icon: FiUser }, { label: "Settings", icon: FiSettings }, { label: "Sign Out", icon: FiLogOut, danger: true }];
    ...
    <>
      <aside className="pd-sidebar">
        <div className="pd-sidebar-inner">
          ...
          <nav className="pd-sidebar-account" aria-label="Account navigation">{accountNavigation.map(({ label, icon: Icon, danger }) => <a className={`pd-nav-link ${danger ? "is-danger" : ""}`} href={`#${label.toLowerCase().replace(" ", "-")}`} key={label}><Icon /><span>{label}</span></a>)}</nav>
        </div>
      </aside>
